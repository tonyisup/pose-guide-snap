...
**Status:** Product and architecture defaults are approved, including the current app-private capture-authority revision that supersedes the earlier MediaStore-authoritative wording. Tasks 1–11B are implemented and committed. Task 9 uses direct bundled MoveNet/LiteRT after exact-artifact review proved MediaPipe Tasks Core's mandatory Google metrics path incompatible with the no-analytics/no-network contract. Task 10's reviewed camera slice is committed at `605c904`; Task 11A's reviewed Room capture authority is committed at `5335466`; and Task 11B's reviewed transactional reference-import backend is committed at `d368e96`. Their authorized Pixel evidence remains bounded and does not complete Gate 2. Task 12 shoot creation and playlist editing is the next sequential slice. This revision supersedes every MediaPipe-specific instruction below.
