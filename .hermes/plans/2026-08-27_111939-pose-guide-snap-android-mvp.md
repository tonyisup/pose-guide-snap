...
**Status (updated 2026-09-17):** Product and architecture defaults remain approved, including the app-private capture authority that supersedes earlier MediaStore-authoritative wording. Tasks 1–12, 14A.1–14A.3, 14B.1B–14B.3, and the Task 15A manual guided-capture slice are implemented and boundedly verified; the residual-journal correction is committed at `382a659`. The exact Task 15A artifact passed its bounded 4/4 Pixel 6 checkpoint, including one real rear-camera three-photo capture through private-file durability, Room confirmation, and one advance. Tasks 16A–16G localize the live detector failure to positive but sub-threshold person/keypoint output. Task 16H's exact Pixel 6 full-body and empty-scene sequences each passed with 97 frames, but their detector-score distributions overlapped completely. Task 16I reused those binaries over wireless ADB in better lighting and obtained 96/96 one-person frames with all 17 landmarks qualified; Task 16J then obtained 97/97 no-person frames from the same-setup empty view. Task 16K evaluated 97/97 intended reference-match frames but never locked; framing failed throughout. Task 16L's component diagnostic passed 3/3 synthetic checks and 1/1 camera collection. Center similarity `0.381554–0.386197` limited all 97 frames, while scale `0.691971–0.711420` also failed `0.800`; Task 16M improved centering to `0.787428–0.793429`, but scale `0.663030–0.668498` and remaining pose criteria still failed. Its 97-frame collection and cleanup passed; Task 16N implements an opt-in live alignment guide with a reference-aspect viewport. Build/lint and 722/722 JVM checks pass; both synthetic native screen checks now pass on the Pixel; the guided camera check collected 96 one-person frames, all unevaluable with only 8–10 landmarks and two torso anchors. Exact cleanup passed. The participant could not see the rear-camera display; Task 16O adds spoken calibration guidance and complete-body readiness. Build/lint, 728/728 JVM checks and 1/1 Pixel audio-only check pass. Clear audibility is participant-confirmed; the first spoken camera attempt stopped at complete-body readiness without a report; Task 16P then collected 95 fully evaluated landscape frames with centering passing throughout; scale and pose criteria still failed and no lock occurred. Cleanup passed; assess spoken-cue preparation time before another run; production audio remains open. Match values remain uncalibrated and automatic capture stays disabled. A 15-minute current-artifact performance run, complete production sequence, audio, export/deletion execution, and remaining acceptance gates are still open. Gate 2 remains incomplete, and no product-shipped claim is made. Standalone Task 13 speech remains deferred until the complete Task 15 coordinator exists. This revision supersedes every MediaPipe-specific instruction below.
...
**Status:** Implementation checkpoints through Task 4 were verified before the complete 43-path Task 14B.1A foundation landed at `57b33c9`. Final complete-candidate specification review later returned `REQUEST_CHANGES` for a malformed byte-equivalent `BLOB` residual-journal key that ordinary text equality could miss. The query-and-regression repair is committed at `382a659`; its focused Pixel regression and cumulative host gate pass, but that does not establish a missing complete-candidate same-digest review. The exact three-file Task 4 Android-test candidate v5, SHA-256 `377dc02c781ece2cf78e48f93c727d02ea9d41082a12229ff22803e97a306491`, received specification `PASS` and engineering/security `APPROVED` without byte drift.
...
This yielded the core ownership boundary and causal tests without touching private device data. Task 9 then added the reviewed direct MoveNet/LiteRT boundary. Task 10 added the committed, authorized-Pixel CameraX pose slice at `605c904`, including the fixed attributed reference, aligned live/reference skeletons, and internal candidate-capture mechanics. Task 11A added committed Room capture authority at `5335466`; Task 11B added the committed transactional reference-import backend at `d368e96`; Task 12 added committed Room V3 preparation plus the create → Photo Picker import → validate → reorder → durably start workflow through `5bc15c3`; and Tasks 14A.1–14A.3 added atomic exact-session reconstruction, active-session discovery, and stale-safe UI admission through `6997861487d3d7dd31c21b9a947d1ccde3b2f418`. The complete 43-path Task 14B.1A foundation then landed at `57b33c9`; its byte-equivalent residual-journal repair is committed at `382a659`. Local Tasks 14B.1B–14B.3 add per-file admission, journal-derived confirmation, settlement, exact private-file effects, and startup recovery. Local Task 15A composes the bounded manual three-photo path and passes its corrected exact 4/4 Pixel checkpoint. Tasks 16A–16G localize the weak live-camera response beyond static model health and black/flat input; Task 16H collected the needed exact detector-score distributions, which overlapped between the full-body and empty-scene conditions. Gate 2 remains unpassed, and repository landing is not a product-shipped claim. Automatic capture, successful positive calibration evidence, the complete Task 15 sequence, sustained performance, and standalone speech remain deferred.
