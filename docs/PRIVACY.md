...
> **Project status: Tasks 1–10 are committed; Task 10 is hardware-exercised.** The app requests `android.permission.CAMERA` plus AndroidX's app-signature permission `com.tonyisup.poseguidesnap.DYNAMIC_RECEIVER_NOT_EXPORTED_PERMISSION`; it requests no `INTERNET`, storage, location, audio, or foreground-service permission and includes no network/cloud/analytics library. It bundles one attributed public reference and fixed public landmark observation. Pixel validation retained no private images, screenshots, or raw landmarks; live analysis frames were not persisted, and candidate/temp capture directories had zero residue after acceptance cleanup. User reference import (Task 11B), Room confirmation/deletion authority (Task 11A), MediaStore export/coordinator integration (Task 14), TTS, deletion UI, and the complete lifecycle below remain unimplemented.
...
One Room transaction then confirms the attempt, records all three authoritative private outputs, advances the pose exactly once, marks the unique confirmation/advance receipt applied, and creates one durable MediaStore export outbox with exactly three output rows. The output table enforces composite identity `(commandToken, burstOrdinal)`, ordinal 0–2, and exactly-three cardinality is verified before commit. If that transaction fails, there is no advance, applied receipt, or outbox; unconfirmed files are cleaned or quarantined. Startup resolves deterministic unconfirmed files before retry and blocks automatic recapture if cleanup cannot be proven.

For each committed output, a Room compare-and-set from `pending` to `claimed` with a unique claim token grants the sole authority to call `MediaStore.insert()`. A claimed/create-started row without a durably recorded exact URI is never returned to pending and never automatically created again after timeout or restart; it is reconciliation-required. The exact target collection/volume and intended metadata are retained only for diagnostics. Display name and relative path are not unique and never authorize update, delete, or reconciliation. After `insert()` returns, the exact URI is persisted before any fallible publication step. Automatic MediaStore update/delete is allowed only via that exact URI; ambiguity preserves potentially foreign rows.
