...
> **Project status: Tasks 1–9 committed; Task 10 is an uncommitted camera-slice candidate.** The candidate requests `android.permission.CAMERA` plus AndroidX's app-signature permission `com.tonyisup.poseguidesnap.DYNAMIC_RECEIVER_NOT_EXPORTED_PERMISSION`; it requests no `INTERNET`, storage, location, audio, or foreground-service permission and includes no network/cloud/analytics library. It bundles one attributed public reference and fixed public landmark observation; live analysis frames are not persisted, app-private candidate captures are excluded from backup/transfer, and the candidate has authorized Pixel 6 evidence. User reference import, Room confirmation, MediaStore export, TTS, deletion, and the complete lifecycle below remain planned behavior for later tasks.
