...
> **Project status: Tasks 1–12 and 14A.1–14A.3 are implemented and boundedly verified. The Task 14B.1A Room V4 journal foundation landed at `57b33c9`, but final specification review returned `REQUEST_CHANGES` after malformed byte-equivalent `BLOB` residual authority was treated as absent; an uncommitted local fail-closed repair is under review.** The app requests `android.permission.CAMERA` plus AndroidX's app-signature permission `com.tonyisup.poseguidesnap.DYNAMIC_RECEIVER_NOT_EXPORTED_PERMISSION`; it requests no `INTERNET`, storage, location, audio, or foreground-service permission and includes no network/cloud/analytics library. Room V4 persists deterministic private relative capture paths and optional byte/hash/timestamp/failure evidence as local authority metadata, with redacted diagnostics. It enables no file I/O, camera capture, MediaStore behavior, new personal-media access, network, analytics, or complete guided lifecycle; no product-shipped claim is made.
...
The planned automatic and manual capture path uses one protocol. One command token identifies exactly three app-private outputs with `(commandToken, burstOrdinal)` identities for ordinals 0–2. For each output, the future file adapter must exclusively create an empty deterministic final reservation, write and sync a same-directory temporary file, verify the reservation's stable filesystem identity, atomically replace only that owned reservation with complete bytes, and sync the directory. The capture-candidates directory is reserved for the future publisher and reconciler; every supported in-process mutation must share one process-wide guard. A pre-existing final may never be clobbered. An empty reservation is non-authoritative and any crash leftover or ownership mismatch must become reconciliation-required; filename presence alone never permits confirmation or recapture. Code that bypasses this ownership boundary is unsupported and must not mutate the directory. Only each publication is atomic; the three-file set is not. The attempt cannot be confirmed until all three non-empty private files exist durably.

Task 11A implements the Room transaction contract that confirms an already-durable capture attempt, records all three authoritative private outputs, advances the pose exactly once, marks the unique confirmation/advance receipt applied, and creates one durable MediaStore export outbox with exactly three output rows. The output table enforces composite identity `(commandToken, burstOrdinal)`, ordinal 0–2, and exactly-three cardinality is verified before commit. The Task 14B.1A candidate deliberately blocks direct first application for unfinished `REGISTERED` and `CAPTURING` attempts before caller output/export metadata traversal or mutation. Journal-owned first application remains deferred to Task 14B.1C.

Task 14B.1A persists capture-file intent, not file authority. New registration atomically creates one attempt and exactly three `EXPECTING_RESERVATION` rows with token-derived private relative paths; V3→V4 migration creates no rows for migrated attempts and preserves all existing authority. `Started` is logical Room state only and conveys no camera, directory, per-file, reservation, write, rename, sync, or cleanup capability. No production per-file effect-admission or journal-transition API exists. Stable initial rows remain unchanged and non-blocking during deletion while their storage shape, ownership, keys, and causal clocks are validated fail-closed.
