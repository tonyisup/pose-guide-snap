...
> **Project status: Tasks 1–12, 14A.1–14A.3, Task 14B.1B–14B.3, Task 15A manual guided capture, and Tasks 16A–16H are implemented. The last host gate passed 728/728 JVM and 14/14 Python tests with zero lint errors.** Task 16I reused the Task 16H scalar-only collector over wireless ADB in better lighting, obtaining 96/96 one-person frames. Task 16J then obtained 97/97 no-person frames in the same-setup empty-scene control; exact device cleanup completed after each run. Wireless ADB is a device debugging transport; app permissions and data boundaries are unchanged. Task 16L's explicitly selected test method collected 97 evaluated frames and emitted only scalar center/scale extrema plus evaluated/unavailable counts; exact device cleanup completed. No image, coordinate, or body extent is retained. The app requests camera plus AndroidX's app-signature permission and requests no internet, storage, location, audio, or foreground-service permission. These slices perform no MediaStore operation, provider scan, network request, analytics, or audio capture; no product-shipped claim is made. Task 16M collected 97 fully evaluated frames with the flash cue: centering improved, but scale and pose criteria still prevented a lock. Exact cleanup passed; live alignment feedback is next. See the [Task 16M record](validation/2026-09-17-task16m-recentered-reference-match-pixel6.md).

## Privacy boundary

The September 4 recovery integration runs the existing reference-import reconciler before loading or retrying the editor. It uses persisted exact import/file identities to clean incomplete imports or quarantine ambiguous outputs. It does not reopen provider images or infer ownership by scanning filenames. An incomplete repair remains visibly blocked. See the [development checkpoint](DEVELOPMENT_PROGRESS.md) for execution limits.
...
| Capture attempt, private-output, and receipt records | Command token, trigger type, session/pose ownership, private output ordinal/path/durability, confirmation/advance receipt, score summary, lifecycle including failed-cleaned or reconciliation-required state, and timestamps | Backup/transfer-excluded app-private Room database; the receipt is unique per command token | Failed-cleaned attempts retain only non-file history and permit same-pose retry. Other records are removed only after the deletion barrier resolves in-progress work safely; reconciliation-required metadata and a minimal tombstone remain until explicitly resolved. |
| Capture-file operation journal | Exactly three rows for each newly registered attempt: token/ordinal ownership, deterministic private relative final/temp/quarantine paths, closed stage, optional byte count/SHA-256/capture timestamp, failure/reconciliation state, and causal timestamps | Backup/transfer-excluded app-private Room V4 database; entity, snapshot, and result diagnostics redact private values | `EXPECTING_RESERVATION` is intent only. Admitted stages block deletion until a Room settlement reaches a stable stage. Clean failure consumes untouched or fully cleaned rows; ambiguous coherent failure retains all three as blocking reconciliation authority. Successful confirmation consumes exactly three final rows after copying their evidence into immutable private-output authority. Task 14B.3 effects use only the exact persisted identity and a cleanup-only recovery lane. |
...
The manual capture path and planned automatic path use one protocol. One command token identifies exactly three app-private outputs with `(commandToken, burstOrdinal)` identities for ordinals 0–2. For each manual output, the coordinator first admits `WRITING_TEMP` in Room, then the file adapter exclusively creates an empty deterministic final reservation and same-directory temporary file before CameraX writes. It syncs and hashes positive bytes, verifies the reservation's stable filesystem identity, admits publication in Room, atomically replaces only that owned reservation, and syncs the directory. Every supported in-process mutation shares one process-wide guard. A pre-existing final may never be clobbered. An empty reservation is non-authoritative and any crash leftover or ownership mismatch becomes reconciliation-required; filename presence alone never permits confirmation or recapture. Only each publication is atomic; the three-file set is not. Confirmation waits until all three non-empty private files are durable.

Task 11A introduced the Room transaction contract for capture confirmation. Task 14B.1C now makes its first application journal-owned: the public call supplies no private paths, sizes, hashes, or capture times. Exactly three coherent `FINAL_DURABLE` rows provide that evidence; the same transaction copies it into immutable private-output rows, advances the pose exactly once, records the unique confirmation/advance receipt, creates one durable MediaStore export outbox with exactly three output rows, and consumes the transient journal. Missing, partial, conflicting, reconciliatory, malformed, residual, or causally newer journal authority fails closed without mutation.

Task 14B.1A persists capture-file intent. New registration atomically creates one attempt and exactly three `EXPECTING_RESERVATION` rows with token-derived private relative paths; V3→V4 migration creates no rows for migrated attempts and preserves all existing authority. `Started` is logical Room state only and conveys no camera, directory, reservation, write, rename, sync, or cleanup capability. Task 14B.1B adds a Room-only per-file compare-and-set API: a fresh transition into one of four in-flight stages durably admits that exact future effect, while replay is informational and stale or malformed authority fails closed. Stable stages remain non-blocking during deletion. No implementation in this slice reads or mutates a file.

Task 14B.2 settles only Room authority. Untouched or fully cleaned rows can be consumed atomically into a retryable `FAILED_CLEANED` attempt; ambiguous coherent rows remain attached to `RECONCILIATION_REQUIRED` and block retry across restart. The API receives no path, file bytes, hash, or filesystem observation from its caller, and it does not scan or mutate storage. Missing journal rows on migrated unfinished attempts do not prove cleanup and remain fail-closed without Task 14B.3's bounded filesystem observation.

Task 14B.3 adds that bounded local reconciliation. The store re-derives every final, temporary, and quarantine path from the opaque token and ordinal, confines it to two closed directories below `noBackupFilesDir`, and rejects symbolic links or nonregular entries. Publication verifies an exclusive empty reservation, synced positive temporary bytes, SHA-256 evidence, atomic rename, and directory sync. Recovery deletes only exact evidence-compatible regular files and preserves unrelated siblings. A migrated journal-free attempt becomes retryable only when all nine deterministic paths are absent; any present or ambiguous path remains blocked. This path does not scan a directory, access a provider URI, invoke CameraX, or touch MediaStore.

Task 15A is the first caller that composes CameraX with that journal. The route holds only a redacted selected-reference projection and reducer state; file paths, hashes, and provider identifiers stay behind the coordinator ports. Duplicate callbacks cannot publish or advance twice. Stop waits for accepted work to finish or settle, while process restart uses the same exact cleanup or ready-to-confirm path before camera admission. Confirmation records deterministic export intent but does not insert, update, query, or delete a MediaStore row.
...
Task 14A.3 retains only an opaque started-session handle in a one-shot in-memory navigation capability. The selected shoot re-queries Room before Resume, and the started route re-queries the exact bootstrap before camera composition. The handle is not placed in a route, deep link, `SavedStateHandle`, log string, or UI copy. Process recreation discards it and returns to list-based Room re-discovery.
...
- App-private reference or capture paths.
- Capture-file byte counts, hashes, capture timestamps, failure details, command tokens, or ownership keys when they could expose private authority or correlate private content.
- Raw landmark arrays.
- MediaStore URIs.
...
## Debug calibration speech

Task 16O sends only fixed coaching/phase phrases to an installed English TTS voice whose contract requires no network connection and whose data is not marked uninstalled. No voice download, microphone recording, audio file, private landmark text, routing override or volume change is performed. The debug manifest adds a TTS-service discovery query; merged app permissions remain CAMERA and the app-signature receiver permission. The 1/1 audio-only Pixel check passed; full airplane-mode/route acceptance is still open. See the [speech record](validation/2026-09-25-task16o-spoken-guidance.md).
