...
**Milestone 1 (code landed, device run pending):** the production guided-camera screen now feeds every analyzed frame through the selected reference's match evidence into the shoot reducer; a held lock requests automatic capture through the same journaled three-photo path as manual capture. The lock policy gates on person count, body visibility, landmark coverage, angular similarity (0.85 acquire / 0.80 release), and the blended overall score (0.825 / 0.775) only; the composition score and the separate positional score are still reported but no longer gate. The status line shows the reducer's phase and the overall score, and the three captured photos are shown after confirmation. Host gate: 786/786 JVM tests, 14/14 calibration tool tests, lint clean, debug and androidTest compile. What remains for this milestone is one Pixel 6 run in which the app captures by itself; no such run has happened yet.
