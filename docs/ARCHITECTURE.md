...
CameraX now provides rear preview, CPU image analysis, and internal still-capture mechanics through one shared viewport. The implemented pose boundary runs the exact bundled MoveNet MultiPose Lightning model through direct LiteRT `1.4.2`. Its detector is deliberately blocking and accepts only upright bitmaps; the camera adapter owns one bounded off-UI worker, keep-latest backpressure, a fixed pre-conversion cadence gate, rotation/crop conversion, per-frame failure containment, `ImageProxy` closure, and sequential exactly-three candidate capture. Task 11A implements the separate durable Room capture authority boundary. Task 11B implements transactional reference import through Room V2, exact journaled app-private file operations, off-main MoveNet analysis, and ledger-driven restart recovery. Task 12 migrates that authority to Room V3 and adds the production shoot list, editor, callback-local Photo Picker path, transactional reorder, and durable-start navigation. Tasks 14A.1–14A.2 add atomic exact-session bootstrap and active-session discovery. Task 14A.3 adds editor-scoped stale-safe Resume and one retained started-route owner; only a fresh exact `Ready` bootstrap can compose the camera destination. The Task 14B.1A candidate migrates local persistence to Room V4 and adds journal intent and validation without connecting the camera to any physical file effect. The camera path still has no user-facing shutter and is not connected to Room confirmation, session advancement, or export. DataStore is reserved for small preferences such as voice enablement, speech cadence, dwell duration, and match thresholds.
...
`markCaptureAttemptStarted(...)` is deliberately a logical Room transition from `REGISTERED` to `CAPTURING`. `Started` does not grant CameraX access, filesystem access, reservation creation, file writes, rename/sync permission, or any per-file capability. There is no production per-file effect-admission or journal-transition API in this candidate. Stable `EXPECTING_RESERVATION` rows therefore represent intent only.

Guided bootstrap reads capture-file operations as Packet 2B's ninth authority family in the same Room transaction as shoot, session, poses, attempts, private outputs, receipts, outboxes, and export outputs. `REGISTERED` requires exactly three coherent initial rows; `CAPTURING` requires exactly three coherent allowed rows; `CONFIRMED` requires no transient journal rows. Malformed storage classes, values, ownership, keys, paths, clocks, cardinality, or cross-family graphs fail closed through redacted rejection families.

Confirmation first performs exact command-token attempt lookup and token/pose matching. It then rejects `REGISTERED` and `CAPTURING` unconditionally before caller-list element traversal, receipt lookup, journal reads, session/shoot loads, deletion classification, broader-authority reads, or mutation. On the `CONFIRMED` path, caller snapshot/validation retains its existing precedence before receipt and residual-journal adjudication; any residual journal row fails closed with `JOURNAL_AUTHORITY_INVALID`, and receipt-backed immutable `AlreadyApplied` remains available only for a coherent already-confirmed graph with zero journal rows. Journal-derived first application remains deferred to Task 14B.1C.

The V3→V4 migration is additive: it preserves every preexisting authority value and creates the new table empty rather than fabricating intent for migrated attempts. Migrated unfinished attempts consequently cannot bootstrap or confirm as though journal authority existed. Deletion includes stable journal authority and its causal clocks in precondition/postcondition validation; stable `EXPECTING_RESERVATION` rows remain byte-for-byte unchanged and do not block deletion because no external effect has been admitted.
