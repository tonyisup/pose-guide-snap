...
After `57b33c9` landed, final specification review reproduced a malformed byte-equivalent `BLOB` journal key that ordinary text equality did not count, allowing confirmed replay to return `AlreadyApplied` despite residual authority. The repair committed at `382a659` uses byte-correlated token matching and adds a deterministic Android regression. That exact method failed on `57b33c9`, then passed 1/1 after the repair; the cumulative host/build gate passed with 635/635 JVM tests and zero lint errors. The repair is committed at `382a659`; this record does not establish missing complete-candidate reviews or remote publication status.

This is not a completed guided-capture product. Task 15A provides the user-facing manual Capture control and composes the reducer, Room V4 journal, CameraX writer, exact app-private file effects, journal-derived confirmation, startup cleanup, and one advance. Tasks 16A–16H add bounded calibration and diagnostics; Task 16H collected per-frame maximum person/keypoint score distributions without retaining model tensors or private media, but its positive and empty-scene distributions overlapped. `Started` still grants only logical state, and startup settlement must finish before camera readiness. Automatic capture, MediaStore execution, audio, physical deletion, calibrated matching, and full Pixel acceptance remain open. The current host gate passes 714/714 JVM and 14/14 Python tests with zero lint errors. No calibration or threshold-change claim is made.
