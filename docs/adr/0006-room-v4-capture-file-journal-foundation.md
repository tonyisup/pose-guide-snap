...
- **Status:** Accepted architecture; the Task 14B.1A foundation landed at `57b33c9`, but final specification review returned `REQUEST_CHANGES` for a malformed residual-authority key; the correction is committed at `382a659`, with historical complete-candidate approvals not established by this repository, and no product-shipped claim is made
- **Date:** 2026-09-03
...
Per-file physical-effect admission and reconciliation were implemented later as Task 14B.1B and are recorded in [ADR 0007](0007-room-owned-capture-file-admission.md). Journal-owned first-application confirmation was implemented as Task 14B.1C and is recorded in [ADR 0008](0008-journal-derived-atomic-confirmation.md). Attempt settlement and Room restart reconstruction were implemented as Task 14B.2 and are recorded in [ADR 0009](0009-room-attempt-settlement-and-restart-reconstruction.md). The journaled private-file adapter and bounded startup reconciliation are implemented as Task 14B.3 and recorded in [ADR 0010](0010-journaled-private-capture-files-and-restart-recovery.md); only Task 15 may compose logical start, per-file admission, CameraX writes, durable settlement, and confirmation.
