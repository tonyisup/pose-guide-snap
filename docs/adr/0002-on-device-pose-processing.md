...
The repository now bundles an exact MoveNet MultiPose model and implements a direct LiteRT detector plus pure mapper. Authorized Pixel 6 instrumentation has exercised the fixed public reference, live rear-camera inference, aligned live/reference skeleton rendering, Task 11B app-private reference analysis, and Task 12's final synthetic-state editor semantics. A separate pre-final manual Pixel run observed the user-facing system Photo Picker import path; it is supporting evidence, not final-artifact instrumentation proof. This bounded evidence establishes local import and preparation behavior but not durable capture confirmation/advancement integration, calibration, or the complete guided workflow.
